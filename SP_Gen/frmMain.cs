...
namespace SP_Gen
{
    public partial class frmMain : Form
    {
        ...
        private string CreateDirectory(string DatabaseName, string FolderName)
        {
            ...
            string targetDir = string.Format("{0}\\Output\\{1}\\{2}", appPath, DatabaseName, FolderName);
        ...
        private void GenerateSQL(string DatabaseName, string tableName, DataRow[] rows)
        {
            ...
            {
                txtSQL.Document.Text = SQL_Generator.CreateSelectAllSP(string.Format("{0}{1}_SelectAll", Prefix, tableName), tableName, rows);
                txtSQL.Document.Text += "\r\n\r\n";
            }

            if (GenerateSelectRow)
            {
                txtSQL.Document.Text += SQL_Generator.CreateSelectRowSP(string.Format("{0}{1}_SelectRow", Prefix, tableName), tableName, rows);
                txtSQL.Document.Text += "\r\n\r\n";
            }

            if (GenerateInsert)
            {
                txtSQL.Document.Text += SQL_Generator.CreateInsertSP(string.Format("{0}{1}_Insert", Prefix, tableName), tableName, rows);
                txtSQL.Document.Text += "\r\n\r\n";
            }

            if (GenerateUpdate)
            {
                txtSQL.Document.Text += SQL_Generator.CreateUpdateSP(string.Format("{0}{1}_Update", Prefix, tableName), tableName, rows);
                txtSQL.Document.Text += "\r\n\r\n";
            }

            if (GenerateDelete)
            {
                txtSQL.Document.Text += SQL_Generator.CreateDeleteRowSP(string.Format("{0}{1}_DeleteRow", Prefix, tableName), tableName, rows);
            }
            ...
            if (AutoSaveScript == true)
            {
                ...
                string scriptFileName = string.Format("{0}\\{1}.sql", CreateDirectory(databaseName, "SQL"), tableName);
        ...
        private void GenerateWrapperClassCSharp(string tableName, DataRow[] rows)
        {
            ...
            {
                string databaseName = tvEntities.SelectedNode.Parent.Text;
                string codeFileName = string.Format("{0}\\{1}.cs", CreateDirectory(databaseName, "WrapperClass"), tableName);
        ...
        private void tbtnGenerate_Click(object sender, EventArgs e)
        {
            if (tvEntities.SelectedNode != null && tvEntities.SelectedNode.Level == 1)
            {
                if (tvEntities.SelectedNode != null)
                {
                    ...
                    DataRow[] rows = dsTablesAndColumns.Tables[0].Select(string.Format("Table_Name = '{0}'", tableName));
