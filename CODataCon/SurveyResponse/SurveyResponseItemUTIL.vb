...
<Serializable()> Public Class SurveyResponseItemUTIL
    Inherits SurveyResponseItem
    Public myAnswerList As New List(Of SurveyResponseAnswerItem)
    ...
    '    End Set
    'End Property
    Public ReadOnly Property CurrentQuestionGroup As QuestionGroupItem
        Get
            Try
                If Not Survey Is Nothing Then
                    If Survey.QuestionGroupList.Count = 1 Then
                        Return Survey.QuestionGroupList(0)
                    End If
                    If CurrentQuestionGroupID > 0 Then
                        Return (From i In Survey.QuestionGroupList Where i.QuestionGroupID = CurrentQuestionGroupID Select i).Single()
                    Else
            ...
            Catch ex As Exception
                AppLog.ErrorLog(ex.ToString, "SurveyResponseItemUTIL.CurrentQuestionGroup")
                Return New QuestionGroupItem
            End Try
        End Get
    End Property
    ...
    Public Function SaveXML() As String
        Try
            ...
            If Me Is Nothing Then
                Return String.Empty
            Else
                Dim oXS As XmlSerializer = New XmlSerializer(GetType(SurveyResponseItemUTIL))
    ...
    Private Function BuildQuestionAnswerForm(ByRef myQuestion As QuestionItem, ByRef curSequence As SurveyResponseSequenceItem) As String
        ...
        Dim myCurrentResponseList As New List(Of com.controlorigins.ws.SurveyResponseAnswerItem)

        myCurrentResponseList = myCurrentAnswers.FindAnswersByQuestionID(myQuestion.QuestionID, curSequence.SequenceNumber)


        If myQuestion.QuestionAnswerItemList.Count = 0 Then
            AppLog.ErrorLog(String.Format("Question with No Answers Defined: QuestionShortNM=", myQuestion.QuestionShortNM), "SurveyResponseItemUTIL.BuildQuestionAnswerForm")
        End If
    ...
    Private Sub BuildGroupQuestionsForm(ByRef myTableHeaderRow As TableRow, ByRef myTable As Table, ByVal reqQuestionGroupID As Integer)
        ...
        For Each myQuestion As QuestionItem In myGroupQuestionList
            Try
                If myQuestion.QuestionAnswerItemList.Count = 0 Then
                    ApplicationLogging.ErrorLog(String.Format("Question with No Answers Defined: QuestionShortNM=", myQuestion.QuestionShortNM), "SurveyResponseItemUTIL.BuildGroupQuestionsForm")
                End If
                ...
                QuestionIndex += 1
            Catch ex As Exception
                AppLog.ErrorLog(ex.ToString, "SurveyResponseItemUTIL.BuildGroupQuestionsForm")
            End Try
        ...
        Using myTableHeaderRow As New TableRow() With {.CssClass = "SurveyFormTableHeaderRow"}
            ...
            If CurrentQuestionGroupID < 1 Then
                If Survey.QuestionGroupList Is Nothing Then
                    ...
                    App.AddMessage("SurveyResponseItemUTIL.GetGroupFormAsTable", String.Format("No Questions for SurveyResponseID-{0}", SurveyResponseID))
                Else
                        ...
                        For Each myGroup As QuestionGroupItem In Survey.QuestionGroupList
                            If myGroup.QuestionGroupID = CurrentQuestionGroupID Then
                                ...
                                If Survey.QuestionGroupList.Count > GroupNumber Then
                                    myNextGroupID = Survey.QuestionGroupList(GroupNumber).QuestionGroupID
                                Else
                                    myNextGroupID = GlobalApplicationProperties.INT_SurveyCompleteGroupID
                                End If
                                If GroupNumber > 1 Then
                                    myPreviousGroupID = Survey.QuestionGroupList(GroupNumber - 2).QuestionGroupID
                                Else
    ...
    Private Sub GetFormQuestionAnswers(ByRef Request As HttpRequest, ByRef myQuestion As QuestionItem, ByVal SequenceNumber As Integer)
        ...
        If myQuestion.QuestionAnswerItemList.Count = 0 Then
            ...
            AppLog.ErrorLog("Question is missing Answers", "SurveyResponseItemUTIL.GetFormQuestionAnswers")
        End If
        ...
        If qaValues.Count > 0 Then
            myAnswerList.Add(New SurveyResponseAnswerItem With {.AnswerComment = qaComment,
                                                                 .ModifiedComment = String.Empty,
                                                                 .QuestionID = myQuestion.QuestionID,
                                                                 .SequenceNumber = SequenceNumber,
                                                                 .SurveyResponseID = SurveyResponseID,
                                                                 .ResponseList = qaValues.ToArray()})
        End If
    End Sub
    Private Sub GetFormQuestionReviewAnswers(ByRef Request As HttpRequest,
        ...
        Dim qaComment As String = Request.Form(GetQuestionAnswerComment(SurveyResponseID, SequenceNumber, myQuestion))
        If myQuestion.QuestionAnswerItemList.Count = 0 Then
            AppLog.ErrorLog("Question is missing Answers", "SurveyResponseItemUTIL.GetFormQuestionReviewAnswers")
        End If
        ...
        If Not String.IsNullOrEmpty(qaAnswerItem) AndAlso IsNumeric(qaAnswerItem) Then
            For Each myReview In ReviewStatusList
                If myReview.ReviewStatusID = CInt(qaAnswerItem) Then
            ...
            If NewAnswerReviewList Is Nothing Then
                AppLog.ErrorLog("NewAnswerReviewList is Nothing", "SurveyResponseItemUTIL.GetFormQuestionReviewAnswers")
            Else
                For Each myReview As SurveyReviewStatusItem In ReviewStatusList
                    If myReview.ReviewStatusID = CInt(qaAnswerItem) Then
                        NewAnswerReviewList.Add(New SurveyResponseAnswerReviewItem With {.SurveyResponseAnswerReviewID = CInt(sqarID),
                                                                                         .ApplicationUserRoleID = curUser.ApplicationUserRoleID,
                                                                                         .ModifiedID = curUser.ApplicationUserID,
                                                                                         .ReviewLevel = curUser.ReviewLevel,
                                                                                         .SurveyAnswerID = CInt(sqaID),
    ...
    Public Sub GetFormAnswers(ByVal Request As HttpRequest)
        For Each myQuestion In Survey.QuestionList
            ...
            Next
        Next
        If myAnswerList.Count > 0 Then
            NewAnswerList = myAnswerList.ToArray()
        End If
    End Sub
    ...
    Private Function GetQuestionDisplay(ByRef myQuestion As QuestionItem) As String
        ...
        If SequenceList.Count > 1 Then
            For Each mySequence As SurveyResponseSequenceItem In SequenceList
                For Each myAnswer As SurveyResponseAnswerItem In myCurrentAnswers.FindAnswersByQuestionID(myQuestion.QuestionID, CInt(mySequence.SequenceNumber))
    ...
    Private Function GetAllQuestionsAsTable() As Table
        ...
        For Each myQuestion As QuestionItem In Survey.QuestionList
            Try
                ...
                QuestionIndex += 1
            Catch ex As Exception
                LogHelper.Log(ex, "SurveyResponseItemUTIL.GetAllQuestionsAsTable")
            End Try
        Next
    ...
    Private Function GetAdminReviewFormAsTable(ByRef ReviewStatusList As List(Of SurveyReviewStatusItem)) As Table
        ...
        Return myTable
    End Function
    Public Function GetSurveyResponseHistory() As String
    ...
    Private Function GetSurveyResponseHistoryRow() As TableRow
        ...
        End Using
    End Function
    'Public Shared Function IsValid(ByRef mySR As SurveyResponseItemUTIL) As Boolean
    '    Dim bReturn As Boolean
    ...
    '    Return bReturn
    'End Function
    'Public Function GetSurveyResponseDataSet() As List(Of SurveyResponseDataSetItem)
    '    Return GetSurveyResponseDataSet(Me)
    'End Function
    'Public Shared Function GetSurveyResponseListDataSet(ByRef SurveyResponseList As List(Of SurveyResponseItem)) As List(Of SurveyResponseDataSetItem)
    '    Dim myReturn As New List(Of SurveyResponseDataSetItem)
    '    For Each mySR In SurveyResponseList
    '        myReturn.AddRange(GetSurveyResponseDataSet(mySR))
    '    Next
    '    Return myReturn
    'End Function
    'Public Shared Function GetSurveyResponseDataSet(ByVal mySR As SurveyResponseItem) As List(Of SurveyResponseDataSetItem)
    '    Return (From ai In mySR.AnswerList
    '                      Join q In mySR.Survey.QuestionList On ai.QuestionID Equals q.QuestionID
    '                      Join sq In mySR.SequenceList On ai.SequenceNumber Equals sq.SequenceNumber
    '                      Select New SurveyResponseDataSetItem With {
    '                          .ApplicationID = AppUtility.GetDBInteger(mySR.ApplicationID),
    '                          .ApplicationNM = "ApplicationNM",
    '                          .SurveyID = mySR.Survey.SurveyID,
    '                          .SurveyNM = mySR.Survey.SurveyNM,
    '                          .SurveyShortNM = mySR.Survey.SurveyShortNM,
    '                          .SurveyResponseID = AppUtility.GetDBInteger(mySR.SurveyResponseID),
    '                          .SurveyResponseNM = mySR.SurveyResponseNM,
    '                          .AssignedUserID = AppUtility.GetDBInteger(mySR.AssignedUserID),
    '                          .AssignedUserNM = "AssignedUserNM",
    '                          .SequenceNumber = AppUtility.GetDBInteger(ai.SequenceNumber),
    '                          .SurveyResponseSequenceID = 0,
    '                          .SequenceText = sq.SequenceText,
    '                          .QuestionGroupID = AppUtility.GetDBInteger(q.QuestionGroupMember.QuestionGroupID),
    '                          .QuestionGroupNM = q.QuestionGroupMember.QuestionGroupNM,
    '                          .QuestionGroupShortNM = q.QuestionGroupMember.QuestionGroupShortNM,
    '                          .QuestionGroupWeight = 0,
    '                          .QuestionID = AppUtility.GetDBInteger(ai.QuestionID),
    '                          .QuestionNM = q.QuestionNM,
    '                          .QuestionShortNM = q.QuestionShortNM,
    '                          .QuestionWeight = AppUtility.GetDBDecimal(q.QuestionGroupMember.QuestionWeight),
    '                          .QuestionValue = AppUtility.GetDBDecimal(ai.QuestionValue),
    '                          .QuestionAnswerID = AppUtility.GetDBInteger(ai.QuestionAnswerID),
    '                          .QuestionAnswerNM = ai.QuestionAnswerNM,
    '                          .QuestionAnswerShortNM = "QuestionAnswerShortNM",
    '                          .QuestionAnswerValue = AppUtility.GetDBDecimal(ai.QuestionAnswerValue),
    '                          .AnswerQuantity = AppUtility.GetDBDecimal(ai.AnswerQuantity),
    '                          .AnswerComment = ai.AnswerComment,
    '                          .AnswerDate = AppUtility.GetDBDate(ai.AnswerDate),
    '                          .AnswerWeightedScore = AppUtility.GetDBDecimal(q.QuestionValue * ai.QuestionAnswerValue * q.QuestionGroupMember.QuestionWeight)}).ToList()
    'End Function
    ...
    Public Function IsGroupComplete(ByVal myGroup As QuestionGroupItem) As Boolean
        ...
        Else
            ...
            Dim dScore As Decimal = CType((From gqi In
                                           (From ai In AnswerList
                                            Join q In Survey.QuestionList
        ...
        If bAnswersRequired Then
            GroupAnswerList = (From ai In AnswerList
                      Join q In Survey.QuestionList On ai.QuestionID Equals q.QuestionID
            ...
            If bReturn Or (myGroup.DependentQuestionGroupID Is Nothing) Then
                For Each myAnswer As SurveyResponseAnswerItem In GroupAnswerList
                    For Each myQ As QuestionItem In Survey.QuestionList
