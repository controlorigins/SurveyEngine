...
'Public Class wpm_SeriesData
'    Inherits System.Collections.Generic.List(Of wpm_PointData)
...
'End Class
'Public Class wpm_DataGridColumn
...
'    Property SeriesDataList As New List(Of wpm_SeriesData)
...
'    Property xColumn As New wpm_DataGridColumn
'    Property xColumnSelected As New List(Of String)
'    Property yColumn As New wpm_DataGridColumn
'    Property yColumnSelected As New List(Of String)
'    Property FilterColumn As New wpm_DataGridColumn
'    Property FilterColumnSelected As New List(Of String)
'    Property ValueColumn As New wpm_DataGridColumn
...
'            For Each SelectedYColumn As string In yColumnSelected
...
'                Dim newSeriesData As New wpm_SeriesData
'                Dim iRunningScore As Double
'                Dim iCount As Integer
'                newSeriesData = New wpm_SeriesData
...
'                            Case "Sum"
'                                newSeriesData.Add(New wpm_PointData With {.PointLabel = AxisLabel, .PointValue = Math.Round(iRunningScore, 2)})
'                            Case "Average"
'                                If iCount > 0 Then
'                                    newSeriesData.Add(New wpm_PointData With {.PointLabel = AxisLabel, .PointValue = Math.Round(iRunningScore / iCount, 2)})
'                                Else
'                                    newSeriesData.Add(New wpm_PointData With {.PointLabel = AxisLabel, .PointValue = 0})
'                                End If
'                            Case "Count"
'                                newSeriesData.Add(New wpm_PointData With {.PointLabel = AxisLabel, .PointValue = iCount})
'                            Case Else
'                                newSeriesData.Add(New wpm_PointData With {.PointLabel = AxisLabel, .PointValue = iRunningScore})
'                        End Select
'                    End If
'                Next
...
'        Try
'            Using sw As New System.IO.StreamWriter(sPath, False)
'                Try
...
'    Private Function ReadFromXML(xmlOfAnObject As String) As wpm_Chart
'        Dim myObject As New wpm_Chart()
...
'        Dim reader As System.Xml.XmlReader = New System.Xml.XmlTextReader(read)
'        Try
'            myObject = DirectCast(serializer.Deserialize(reader), wpm_Chart)
'            Return myObject
'        Catch
'            Throw
'        Finally
